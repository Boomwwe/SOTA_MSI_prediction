...
def get_args():
    ...
    parser.add_argument(
        '-s', '--slide', default="/data/gbw/TCGA_CRC_SVS",
                        help='Path to whole-slide image (SVS or JPG format) or folder of images (SVS or JPG) to analyze.')
    parser.add_argument('-o', '--out', default="/data/gbw/TCGA_CRC_pic",
                        help='Path to directory in which exported images and data will be saved. BUT no use in this code, please refer -ps below')
    ...
    parser.add_argument('--ov', type = float, default=1,
    ...
    parser.add_argument('-ps','--pic_save_path',default="/data/gbw/512_orginal_pic",
...
if __name__ == ('__main__'):
    ...
    patient1 = os.listdir(args.slide)
    patient2 = os.listdir(args.pic_save_path)
    inputlist = [i for i in patient1 if not i.endswith('.svs')]
    for i in inputlist:
        save_path = args.pic_save_path+"/"+str(i[0:12])
        #len of patient name = 12
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        try:
            print(i)
            move(args.slide + "/" + i, save_path)
        except:
            print(args.slide + "/"  + str(i) + "/" + str(j))
